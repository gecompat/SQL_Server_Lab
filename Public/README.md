...
| `Sync-SqlServerLabPersistentStorageArtifact` | `Sync-SqlServerLabPersistentStorageArtifact.ps1` | Genau ein vorhandenes `BackupSetId` oder `DatabasePackageId` vollständig revalidieren, mit `-WhatIf` mutationsfrei planen und controllergebunden idempotent registrieren |
