...
| `Get-SqlServerLabReconcilePlan` | Read-only Lifecycle-, eng begrenzter Hyper-V-Netzwerk-, Containerressourcen-/Autostart- oder External-Runtime-Reconcile-Plan |
| `Invoke-SqlServerLabReconcileAction` | Start/Stop, eigentumsgebundene Hyper-V-Netzwerkreparatur, journalisierter Containerressourcen-/Autostart-Reconcile oder validierter additiver Container-Runtime-Refresh (mit `-WhatIf`) |
