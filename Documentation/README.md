...
[Persistentes Retrieval](Architecture/AI_PERSISTENT_RETRIEVAL.md) beschreibt den begrenzten SQL-2025-Containerpfad mit Fixture- oder atomar synchronisierten Caller-Dokumenten, exakter Vektor- und hybrider lexikalischer SQL-Suche, Resume und eigenem Cleanup.

[SQL-seitige HTTPS-Embeddings](Architecture/AI_SQL_HTTPS_BRIDGE.md) beschreibt
den internen Docker-Referenzslice. SQL-Embeddings, TLS-Negative, Retrieval nach
SQLrestart und eigenes Cleanup bestanden nativ am 2026-09-21.
...
| Hyper-V | Lifecycle, sealed Registry und enger Manifestpfad aus SQL-Prepared-Image; echter SQL-2025-Vertical-Slice aus frischem Windows-Slot einschließlich Storage, Ressourcenwechsel, Datenpersistenz und Cleanup akzeptiert; nativer Testdatenbank-Reconcile einschließlich stabiler SQL-Readiness nach VM-Neustart und vollständigem Cleanup akzeptiert | `Providers/HyperV/HyperVProvider.ps1`, `Private/HyperVImageRegistry.ps1`, `../Tests/Integration/Invoke-HyperVCliAcceptance.ps1`, `../Tests/Integration/Invoke-HyperVTestDatabaseReconcileAcceptance.ps1` |
...
| `Repair-SqlServerLabPersistentStorageCatalog` | Einen nachweisbar eigenen, abgetrennten UUID-labelgebundenen Docker-/Podman-Store ohne Runtime- oder Datenmutation revisionsgeschützt wieder katalogisieren |
...
| `Stop-SqlServerLabOvmsHttpsGateway` | Ausschließlich den eigenen sitzungsgebundenen OVMS-Gateway beenden und API-Key bereinigen |
| `Get-SqlServerLabLlamaCppRuntime` | Lokale Windows-/Linux-llama.cpp-Pakete ohne Hashpflicht und Prozessstart erkennen |
...
| `Get-SqlServerLabAiRuntimeCapability` | Unmittelbare llama.cpp-Paketbinärdateien hashen und CPU-/CUDA-/ROCm-/Vulkan-/SYCL-/OpenVINO-Lanes ableiten |
...
| `Measure-SqlServerLabAiComputeCandidate` | Explizit gebundenen llama.cpp-Kandidaten mit identischem Messprofil benchmarken |
| `Measure-SqlServerLabAiComputeCandidateSet` | Alle geeigneten llama.cpp-Kandidaten mit identischem Profil messen und die schnellste vollständige Auswahl liefern |
| `Get-SqlServerLabAiSharedGatewayPlan` | Gemeinsamen lokalen HTTPS-Gateway und mehrere SQL-Verbraucher geheimnisfrei planen; noch keine Ausführung |
...
| `Get-SqlServerLabAiSharedGatewayStatus` | Registrierung, eigenen Sessionbesitz, Plan-/Inhaltsdrift und fremde Portbelegung read-only klassifizieren |
| `Get-SqlServerLabAiSharedGatewayServicePlan` | Persistenten Benutzer-Autostart für Windows S4U oder Linux systemd user read-only planen und fehlende Hostvoraussetzungen exakt ausweisen |
| `Test-SqlServerLabAiSharedGatewayServiceSecret` | Consumer-Schlüssel für den gebundenen Principal nur über SecretManagement auflösen; echter nichtinteraktiver Service-Logon bleibt offen |
...
| `Invoke-SqlServerLabAiExternalModelSqlApply` | Gebundenes Ownership-Receipt, Credential und External Model journalisiert und transaktional erstellen; Teilzustände bleiben Recoverybedarf |
| `Test-SqlServerLabAiExternalModelSqlEmbedding` | Applied-Ownership revalidieren und ein festes SQL-Embedding mit sanitisiertem Receipt prüfen |
...
| `Get-SqlServerLab` | State sowie providerneutralen Container-/VM-Live-Status und sanitisierte kataloggebundene Container-Tool-Metadaten anzeigen; ungeprüfte Hyper-V-SQL-Bereitschaft bleibt davon getrennt |
...
| `Invoke-SqlServerLabAiPersistentRetrieval` | Feste synthetische Generationen oder eine Caller-Collection in SQL Server 2025 persistieren, vollständig und atomar synchronisieren, alte v1-Caller-Generationen explizit begrenzen und per exakter Vektor- oder hybrider lexikalischer/Vektorsuche abfragen; Fixture oder Caller-Bestand explizit nach Nomic migrieren, gebunden fortsetzen und entfernen |
