...
| `CUI-022` | reservierter Statusbereich mit Fortschritt und Heartbeat | `IMPLEMENTED` - feste Bandhöhe, sitzungsweite Vererbung an Menü- und Formularbildschirme, Live-Aktualisierung ausschließlich der reservierten Zeilen, Stillstandserkennung, ASCII-Fallback ohne UTF-8-Konsole |
...
| `CUI-024` | Kontexthilfe je `ScreenId` und Begründung deaktivierter Einträge | `IMPLEMENTED` - alle 108 statisch verwendeten Bildschirme besitzen kuratierte Hilfe; dynamische IDs sind begrenzt gemustert und neue Lücken werden statisch blockiert |
