...
| Point-in-Time-Recovery-Szenario | `implemented_pending_native` | `Invoke-PointInTimeRecoveryAcceptance.ps1` bindet je Docker-/Podman-Lauf einen neuen eigenen SQL-2025-Run, containerlokale Full-/Log-Backups, einen SQL-seitigen Serverzeit-`STOPAT` zwischen gutem Marker und Fehlmutation sowie Restore in ein neues Ziel. Der Runner prueft Quelle, Inhalt, `DBCC CHECKDB`, Recoverydauer und scopegebundenen Cleanup. Offline-Vertraege und CI-Hooks sind vorhanden; getrennte native Provider-Evidence steht noch aus. Kein allgemeiner Scenario-Executor oder oeffentlicher PITR-Restorevertrag. |
