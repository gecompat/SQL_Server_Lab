...
| Recovery-Härtung | `in_progress` | Netzwerk-Cleanup prüft Run-/Scope-Labels. Die neue native Docker-/Podman-Abnahme erstellt je ein eigenes und ein fremdes Netzwerk, blockiert die fremde Löschung, entfernt ausschließlich das eigene über den Produkt-Cleanup und bereinigt das fremde Testnetz danach explizit (je 3 PASS). Prozessabbruch, wiederholtes Cleanup, manipulierte Ownership, Junctions und gemischte Provider-Teilfehler bleiben insgesamt abzuarbeiten; fremde Objekte erhalten und idempotente Wiederaufnahme belegen. |
