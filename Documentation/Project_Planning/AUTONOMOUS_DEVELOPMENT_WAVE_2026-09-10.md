...
| Recovery-Härtung | `in_progress` | Netzwerk-Cleanup prüft Run-/Scope-Labels. Die native Docker-/Podman-Abnahme erstellt je ein eigenes und ein fremdes Netzwerk, blockiert die fremde Löschung, entfernt ausschließlich das eigene über den Produkt-Cleanup und bereinigt das fremde Testnetz danach explizit (je 3 PASS). Ein isolierter nativer Docker-Batchabbruch nach sichtbarer Providerressource bestand: persistierte Worker-Recovery, eindeutige Run-Übernahme, idempotentes Resume und scopegebundener Cleanup (13 PASS). Wiederholtes Cleanup, manipulierte Ownership, Junctions, Podman-Abbruch und gemischte Provider-Teilfehler bleiben abzuarbeiten; fremde Objekte erhalten und idempotente Wiederaufnahme belegen. |
...
| 2026-09-11 | Nativen Docker-Batchabbruch ausgeführt | Nach sichtbarer eigener SQL-2025-Dockerressource wurde der separate Scheduler hart beendet. Persistierte Worker wurden deterministisch übernommen, Resume erzeugte keine doppelten Runs und der Batch-Cleanup entfernte beide Container und Volumes (13 PASS). Podman-Abbruch, Ownership-/Junction-Manipulation und gemischte Teilfehler bleiben offen. |
