...
| Privacy-Scanner und parallele Runtime | `in_progress` | Bei der ANN-Abnahme wurde eine falsche Maskierung der Runtime-Pfadausschlüsse reproduziert. Korrektur trennt flüchtigen State vom Quellscan und prüft dennoch erzwungen versionierte Runtime-Dateien sowie versteckte aktive Env-Dateien. Isolierte synthetische Positiv-/Negativprüfungen und Windows-/Linux-Gates sind vor Integration erforderlich. |
...
|---|---|---|
| Hyper-V-Reconcile | `in_progress` | Isolierte native Runner für SQL-Konfiguration, SQL-Port, Testdatenbanken und External Runtimes ausführen; Ressourcen-, Netzwerk- und Storage-Reparatur separat mit No-op, Mutation, Resume, Persistenz und Cleanup nachweisen. Ein erhöhter Lauf bestätigte auf `d07d3f02` Fresh-VM, OOBE, SQL-2025-PrepareImage, Generalize, immutable Publish und Manifestklon. Die nachgelagerte Aktivierung scheiterte gezielt an fehlendem vorhandenen Egress (`WINDOWS_ACTIVATION_EXISTING_EGRESS_UNAVAILABLE`); VM, Child-VHDX und IPAM-Lease wurden vollständig entfernt. Der Fehlerpfad bereinigt nun auch einen erfolgreichen SQL-Build-State vor der Entfernung des ausschließlich test-eigenen Prepared-Artifact. Host-Egress oder ein abweichender Aktivierungsvertrag sind vor der nächsten Reconcile-Abnahme zu klären. |
...
| Recovery-Härtung | `in_progress` | Erster konkreter Befund: Netzwerk-Cleanup prüfte im Gegensatz zu Volumes keine Run-/Scope-Labels. Für Docker und Podman simuliert reproduziert (4 PASS, 2 FAIL), labelgebunden korrigiert (6 PASS); native Netzwerk-Abnahme offen. Prozessabbruch, wiederholtes Cleanup, manipulierte Ownership, Junctions und gemischte Provider-Teilfehler bleiben insgesamt abzuarbeiten; fremde Objekte erhalten und idempotente Wiederaufnahme belegen. |
