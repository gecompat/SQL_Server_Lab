...
| Welle 4 – Hyper-V Provider | teilweise; nativer Lifecycle, Windows-2025-/SQL-2025-Prepared-Image-Pfad und vCPU-/RAM-Reconcile-Referenz real validiert | Immutable Image-Registry, Baseline-Auswahl, Generation-2-/Secure-Boot-/Parent-Child-VHDX-Lifecycle, Gast-Drives, Windows-Specialization, SQL-`PrepareImage`/`CompleteImage`, SQL-Readiness, portable Network- und vCPU-/RAM-Intents sowie journalisierter Network- und Resource-Reconcile sind implementiert. VM-/VHDX-/Start-/Stop-/Lifecycle-/Cleanup-Pfad sowie ein frischer Build und normaler differenzierender Manifest-Klon bis `SQL_READY_RUN` sind real grün. Am 2026-09-14 bestand die native Windows-2025-/SQL-2025-Enterprise-Referenz für dynamische und statische vCPU-/RAM-Reconcile-Pfade einschließlich `VerifyOnly`, SQL-Readiness, Shutdown-Integration, Datenmarker und Cleanup. Breites Datenbank-/Software-/Post-Provisioning-Binding, weitere Hardware-/Storage-/SQL-Reconcile-Klassen, weitere native Reparaturläufe und die allgemeine vollautomatische OS-Factory bleiben offen |
| Welle 5 – Scenario Engine und Fault Injection | teilweise; interne, eng begrenzte Slices umgesetzt | `SCN-801` liefert ausschließlich nicht ausführbare providerneutrale Metadaten (`ExecutionImplemented=false`). Der private synthetische SCN-802-Executor einschließlich SCN-803-Capability-Entscheid und SCN-804-Phasenfristen ist provider- und SQL-los. Für frische operationseigene Docker-/Podman-Container sind CPU- und Memory-Pulse mit Rücknahme und Hard-Interrupt-Recovery getrennt belegt. Öffentliche API, Provider-/SQL-gebundene Ausführung, Consumer-Migration, Netzwerk-/I/O-/TempDB-/Log-Faults und sämtliche Wellen-Gates bleiben offen. |
