...
Remote-Validierung und sind keine Produktabhängigkeit. Der frühere UX-Punkt
zur Hyper-V-Umgebungsverwaltung ist ebenfalls aufgelöst: Bestehende Hyper-V-
Umgebungen werden unter **Umgebungen verwalten** ausgewählt; das
Infrastrukturmenü enthält Vorlagen-, Slot-, Build- und Medienaufgaben.
...
|---|---|---|
| M0 Statuswahrheit | `validated` (gemäß M0-Statuswahrheitsmatrix) | Statusmatrix bei neuen Planungs- oder Qualitätsänderungen mit ausgeführter Evidence fortschreiben |
...
Die deduplizierten Kriterien, das vollständige Alt-Wellen-Mapping, das
...
**Stand 2026-09-11:** `validated` für den lokalen M0-Scope nach der
kanonischen M0-Statuswahrheitsmatrix. Dokumentations-, Console-UI- und
impacted Checks sind ausgeführt und grün. Der frühere rote
...
Standalone-Einstieg reproduzierbar und `0` beendet ihn kontrolliert. Das reale
Hyper-V-Windows-User-Gate bleibt nach einem read-only Probe ausschließlich
...
`REMOVED`. `ADP-004` wurde mit
[SQL_Server_Analyze PR #112](https://github.com/gecompat/SQL_Server_Analyze/pull/112)
auf `origin/main` (`45a9594`) gemergt. Der Adapter `EXECUTION-PLAN-001`
installiert den eigenständigen Execution-Plan-Analyse-Frameworkteil, erzeugt
synthetische Plan-Evidenz und validiert `USP_ExecutionPlanAnalysis`. Die
