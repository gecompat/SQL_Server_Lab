...
| `AIX-008` | P1 | Backendvergleich und Auswahl | `IMPLEMENTED_PARTIAL`: Windows-/Linux-Hostinventar mit expliziter CPU-/GPU-/NPU-Coverage, inhaltsgebundene llama.cpp-Pakete, herstellergefilterte CPU-/CUDA-/ROCm-/Vulkan-/SYCL-/OpenVINO-Lanes und vollständige Kandidatenbildung sind implementiert. `Measure-SqlServerLabAiComputeCandidate` führt `llama-bench` offline aus, bindet GGUF, Paket, Profil, Inventar und Kandidat und liefert Durchsatz, P95-Latenz sowie Peak Working Set. `Measure-SqlServerLabAiComputeCandidateSet` ordnet alle benötigten Pakete über den Runtimehash zu, misst sämtliche geeigneten Optionen mit identischem Profil und liefert nur bei vollständiger Coverage direkt die schnellste Auswahl. Runtime-Geräteselektoren werden standardmäßig aus der aktuellen eindeutigen `--list-devices`-Ausgabe abgeleitet; ein explizites Binding bleibt möglich und mehrdeutige Teilmengen fallen geschlossen aus. Pinned fixiert weiterhin ohne Benchmarkentscheidung. Der opt-in Acceptance-Runner erzeugt nach vollständiger Messung ein schema-validiertes pfadfreies Receipt; native Evidence ist noch ausstehend. Der eigene Windows-/Linux-llama.cpp-Start konsumiert die Auswahl hashgebunden einschließlich Mehr-GPU; Linux bindet Ownerverlust über `setpriv --pdeathsig` und Listener über `/proc`. Energieattestation, native Matrix und echte Linux-Lifecycle-/ROCm-Evidence bleiben offen. |
...
Der konkrete [Windows-/Linux-Ownershipvertrag](../Architecture/LLAMA_CPP_OWNED_RUNTIME.md) bindet Start/Stop, Lease, Endpointprobe und optionales Artifact-Hashing.
