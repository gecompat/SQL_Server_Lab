...
| `AIX-007` | P1 | OpenVINO Model Server | `IMPLEMENTED_PARTIAL`: Die read-only Probe bindet einen vorhandenen OVMS-v3-Upstream an numerischen Loopback, exakten Modellnamen, Antwortform und Dimension. Der getrennte Windows-TLS-Gateway-Lifecycle ist mit Ownerkanal, Lease, Auth, Request-/Response-Rekonstruktion und synthetischer End-to-End-Abnahme implementiert. SQL-Pläne bleiben mit `AI_EXTERNAL_MODEL_OVMS_GATEWAY_NOT_IMPLEMENTED` blockiert, bis sie den Gateway-Binding-Key übernehmen und die SQL-/Acceleratorabnahme besteht. |
