...
| `AIX-007` | P1 | OpenVINO Model Server | `IMPLEMENTED_PARTIAL`: Eine read-only Probe bindet einen vorhandenen OVMS-v3-Upstream an numerischen Loopback, exakten Modellnamen, Antwortform und Dimension. OVMS verschlüsselt REST/gRPC nicht selbst. SQL-Pläne bleiben mit `AI_EXTERNAL_MODEL_OVMS_GATEWAY_NOT_IMPLEMENTED` blockiert, bis der getrennte TLS-Gateway-Lifecycle implementiert und abgenommen ist; der vorhandene Ollama-Referenzgateway ist dafür nicht allgemein freigegeben. |
