...
| `AIX-008` | P1 | Backendvergleich und Auswahl | `IMPLEMENTED_PARTIAL`: Windows-/Linux-Hostinventar mit expliziter CPU-/GPU-/NPU-Coverage, inhaltsgebundene llama.cpp-Pakete, herstellergefilterte CPU-/CUDA-/ROCm-/Vulkan-/SYCL-/OpenVINO-Lanes und vollständige Kandidatenbildung sind implementiert. `Measure-SqlServerLabAiComputeCandidate` führt `llama-bench` offline mit expliziten Runtime-Geräteselektoren aus, bindet GGUF, Paket, Profil, Inventar und Kandidat und liefert Durchsatz, P95-Latenz sowie Peak Working Set. Der Auswahlkern verwendet Auto als Default und verlangt vollständige vergleichbare Evidence; Pinned fixiert ohne Benchmarkentscheidung. Automatische portable Zuordnung von Hardware-ID zu Runtime-Selector, Energieattestation, native Matrix und Consumerintegration bleiben offen. |
