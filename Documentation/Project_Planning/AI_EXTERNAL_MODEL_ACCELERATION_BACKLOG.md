...
| `AIX-008` | P1 | Backendvergleich und Auswahl | `IMPLEMENTED_PARTIAL`: Windows-/Linux-Hostinventar mit expliziter CPU-/GPU-/NPU-Coverage und datenschutzbegrenztem Hash sowie vollständige Kandidatenbildung aus strikten Runtime-Fähigkeiten sind implementiert. Der Auswahlkern verwendet Auto als Default, verlangt vollständige vergleichbare Evidence aller geeigneten Einzel-/Mehr-GPU- und gemischten Sets und rangiert Durchsatz, P95-Latenz, Arbeitsspeicher sowie stabile ID. Pinned fixiert einen geeigneten Kandidaten ohne Benchmarkentscheidung oder Fallback. Automatische Runtime-Fähigkeitsableitung, Benchmarkproducer, Energie-/Geräteattestation und Consumerintegration bleiben offen. |
