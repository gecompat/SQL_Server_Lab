...
| `AIX-007` | P1 | OpenVINO Model Server | `RESEARCHED`: OpenAI-kompatible Embeddings und explizites `--target_device NPU` sind vorhanden. OVMS verschlüsselt REST/gRPC nicht selbst. Pläne bleiben mit `AI_EXTERNAL_MODEL_OVMS_GATEWAY_NOT_IMPLEMENTED` blockiert, bis ein OVMS-spezifisch gebundener Gateway-Lifecycle implementiert und abgenommen ist; der vorhandene Ollama-Referenzgateway ist dafür nicht allgemein freigegeben. |
