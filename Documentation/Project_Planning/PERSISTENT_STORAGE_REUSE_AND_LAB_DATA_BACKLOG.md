...
sind journalisiert und idempotent. Auch diese Operations-Writer verwenden
jetzt den gemeinsamen Mutationskern mit Preview und erwarteter Revision.
Container-Clone-Lease und Zielregistrierung,
regulaerer Container-Lease-Erwerb und -Release sowie der
Writer fuer verifizierte Container-Datenbankreferenzen nutzen jetzt denselben
Mutationskern mit read-only Preview und optionaler erwarteter Revision.
Release persistiert eine erforderliche Recovery-Markierung vor dem Fehler;
Preview schreibt sie nicht. Stabile Referenz-IDs, exklusiver Run-/Scope-Lease-Schutz,
...
dadurch keine Delete-Autorität. Der getrennte UUID-basierte Retained-Store-Delete
ist unten beschrieben; seine native Abnahme steht aus. FILESTREAM, TDE und weitere
endgültige Storage-Löschungen bleiben blockierte Folgearbeit. Der
...
- Zustand `AVAILABLE`, `IN_USE`, `DETACHED`, `INCOMPLETE`, `RECOVERY_REQUIRED`
  oder `DELETE_PENDING`; nach der getrennten bestätigten Löschung bleibt die ID als `REMOVED`-Tombstone reserviert;
...
| `PSR-011` | P1 | identische CLI- und GUI-Flows für Auswahl, Retention, Restore, Attach, Clone und Delete liefern | `IMPLEMENTED_PARTIAL`: Backup-Inventur/Restore, Container-Continue/Clone, Retention-Vorschau, Retain/Backup-on-Remove sowie pfadfreie Datenbankpaket- und Hyper-V-Daten-VHDX-Auswahl verwenden in CLI und Browser dieselben stabilen IDs und Fachkerne; Hyper-V-Release/Reattach/Clone und Datenbankpaket-Attach laufen über dieselben öffentlichen ID-/Run-gebundenen Actions; der öffentliche finale Delete des rungebundenen Executors ist eng auf den revisionsgebundenen Docker-/Podman-`INSTANCE_STORE`-`DELETE_WITH_RUN`-Pfad beschränkt; weitere Paketprovider und breitere endgültige Löschungen bleiben offen; der getrennte ID-/Preview-/Resume-Pfad für einzelne moderne eigene detached Docker-/Podman-Stores mit dauerhaftem `REMOVED`-Tombstone ist implementiert, seine native Abnahme steht aus |
...
Stand 2026-09-04: `SqlServerLab.DatabaseMigrationDependencyInventory/1.0`
...
`DATABASE_FILES_ONLY` sowie `FullInstanceMigration=false`. Die lokale Browser-
UI kann dieselbe read-only Inventur für eine laufende Containerinstanz nur per
Run-/Instanzbindung, validiertem Datenbanknamen und flüchtigem SA-Passwort
auslösen; ihr Live-Log projiziert ausschließlich Kategorien, Counts,
Migrationsgrenzen und erforderliche Review-Schritte. Ohne verifizierte
...
### Enger endgültiger Retained-Store-Delete (PSR-004 / PSR-011)

`Get-SqlServerLabRetainedStoreRemovalPlan` und
`Invoke-SqlServerLabRetainedStoreRemoval` implementieren den getrennten
CLI-/Browser-/API-Pfad für einen einzelnen modernen eigenen detached
Docker-/Podman-Instanzstore. Revision und PlanKey binden die vollständige Original-
Evidence und aktuelle Runtime; `DELETE_PENDING` reserviert gegen parallele
Continue-/Clone-/Recovery-Aufrufe. Ein atomisches Journal ermöglicht ausschließlich
vorwärtsgerichtetes Resume. Bestätigte Abwesenheit endet in einem unveränderlichen
`REMOVED`-Tombstone; alte Register-/Recovery-Aufrufe dürfen diese ID nicht wiederverwenden.

Die Preview nennt historische Datenbankreferenzen, bestätigt aber kein vollständiges
aktuelles Offline-Inhaltsinventar und kein Backup. Alle SQL-Inhalte des Stores gehen
verloren. Sidecars, Gruppen, Legacy ohne vollständige Evidence, externe Speicher und
