...
| SCENARIO_CONTRACT_BACKLOG.md | Interner, nicht ausführbarer `SCN-801`-Metadatenslice für Scenario-, Step-, Evidence- und Outcome-Metadaten (`ExecutionImplemented=false`), mit getrennten privaten synthetischen SCN-802–804-Ergänzungen; öffentliche Contracts, Provider-/SQL-Ausführung und Consumer-Migration bleiben offen |
...
| SQL_GUEST_EVALUATION_EVIDENCE_BACKLOG.md | Minimaler Hyper-V-Vertrag für versionsgebundene, frische und geheimnisfreie SQL-Gast-Evaluations-Evidence; Reader und Watch-Projektion sowie enger Developer-Capture nativ belegt, positive Deadline-Abnahme offen |
| [CROSS_CUTTING_PLATFORM_CAPABILITIES_BACKLOG.md](CROSS_CUTTING_PLATFORM_CAPABILITIES_BACKLOG.md) | Vollständige CLI-Abdeckung aller Lab-Funktionen über `Invoke-SqlServerLab`, einschließlich nachträglicher Änderungen und Skriptaufrufe; außerdem Evaluation-Watchdog, Gesamt-Lab-Portabilität, externe Secret Stores, Observability, Recovery Points, Framework-Upgrades, Air Gap, Kapazitätssteuerung, Mehrbenutzerbetrieb und Automation-API/IaC |
