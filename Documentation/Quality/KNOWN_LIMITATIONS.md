...
`bge-m3:latest`, `nomic-embed-text-v2-moe:latest` oder `all-minilm:latest`
voraus. Er
...
Assertions und vollständig bestätigtem Cleanup. Die All-MiniLM-/384-Auswahl
bestand am selben Tag mit gebundenem Hybridmodus, denselben sechs Assertions
und vollständig bestätigtem Cleanup. Andere Provider, weitere Modelle und
...
Modell explizit den implementierten Hybridmodus; beliebige deutsche
Retrievalqualität wird daraus nicht abgeleitet.
...
`embeddinggemma:latest`, `nomic-embed-text:latest`, `bge-m3:latest` oder
`all-minilm:latest` mit exakter SQLsuche und
...
2026-09-22 getrennt mit je 30 Assertions einschließlich Hybridranking,
SQLrestart, Delta-Cutover, Caller-Sync, explizitem Prune, Hashdrift und vollständigem Cleanup. SQL Server
...
Referenzen verwendeten jeweils drei Dokumente. Alte inaktive Generationen einer
v1-Caller-Collection können nach erfolgreichem Sync explizit und transaktional
begrenzt werden. Die monotone Generationsnummer wird dadurch nicht zurückgesetzt;
automatische Retention, Prune nach Modellmigration und mehr als 32 Generationen sind offen. Der
...
und All-MiniLM. BGE-M3-, All-MiniLM- und direkte Nomic-v2-Collections können
...
Diese Assertion und ihr statischer Vertrag sind korrigiert. Der erhöhte
Wiederholungslauf vom 2026-09-12 bestätigte danach Addition, Journal,
eigentumsgebundene Entfernung, LAB_GENERATED-Wiederverwendung, VM-Neustart und
stabile Host-SQL-Readiness nach 94,2 Sekunden. Der anschließende
scopegebundene Cleanup entfernte Run, VM, VHDX, Prepared-Artifact und
testlokalen State Root vollständig. Dieser positive Nachweis gilt für den
ausführbaren Testdatenbank-Reconcile-Vertrag; weitere native Reconcile-Klassen
bleiben getrennt zu belegen.
...
`BACKUP_AND_PACKAGE` für MDF/NDF/LDF-Dateien sowie `EXTERNAL_UNMANAGED` für
eine ausschließlich katalogisierte Bindungsfreigabe. Die Kombinationspolicy
...
beziehungsweise Recovery-Key-Vertrag vor der Offline-Mutation fail-closed.
Jeder read-only Removal-Plan
