...
process {
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            if ($runtimePath -match '(?i)(HyperVSampleManifest|hyperv-sample-manifest|HyperVResourceReconcile|hyperv-resource-reconcile)') { $pathRuntime.HyperV = $true }
