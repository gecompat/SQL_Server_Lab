...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiSharedGateway(Plan|Preflight|Registration|Storage|Upstream|Session)|Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGateway(Preflight|Upstream)|Register-SqlServerLabAiSharedGatewayStorage|(Start|Stop)-SqlServerLabAiSharedGatewaySession|ai-shared-gateway-(plan|preflight|storage|upstream|session))'; Checks = @('Invoke-AiSharedGatewayPlanChecks.ps1','Invoke-AiSharedGatewayPreflightChecks.ps1','Invoke-AiSharedGatewayStorageChecks.ps1','Invoke-AiSharedGatewayUpstreamChecks.ps1','Invoke-AiSharedGatewaySessionChecks.ps1','Invoke-AiExternalModelAccelerationChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiExternalModelPlanPath = $runtimePath -match '(?i)^(Private/(AiExternalModelAcceleration|AiOvmsHttpsGateway)\.ps1|Public/(Get-SqlServerLabAiExternalModelPlan|Test-SqlServerLabAiExternalModelEndpoint|Test-SqlServerLabAiExternalModelArtifact|Test-SqlServerLabOvmsUpstreamEndpoint|Start-SqlServerLabOvmsHttpsGateway|Stop-SqlServerLabOvmsHttpsGateway)\.ps1|Tools/Invoke-OvmsHttpsGatewayWorker\.ps1|Schemas/(ai-external-model-(plan|endpoint-receipt|artifact-receipt)|ai-ovms-upstream-receipt)\.schema\.json|Tests/(Static/Invoke-AiExternalModelAccelerationChecks|Integration/Invoke-AiOvmsHttpsGatewayAcceptance)\.ps1)$'
            $aiSharedGatewayPlanPath = $runtimePath -match '(?i)^(Private/AiSharedGateway(Plan|Preflight|Registration|Upstream|Session)\.ps1|Public/(Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGateway(Preflight|Upstream)|Register-SqlServerLabAiSharedGatewayStorage|(Start|Stop)-SqlServerLabAiSharedGatewaySession)\.ps1|Tools/Invoke-AiSharedGatewaySessionWorker\.ps1|Schemas/ai-shared-gateway-(plan|preflight-receipt|storage-(state|receipt)|upstream-receipt|session-receipt)\.schema\.json|Tests/Static/Invoke-AiSharedGateway(Plan|Preflight|Storage|Upstream|Session)Checks\.ps1)$'
