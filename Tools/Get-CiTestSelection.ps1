...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiExternalModelAcceleration|AiExternalModelPlan|ai-external-model-plan)'; Checks = @('Invoke-AiExternalModelAccelerationChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiExternalModelPlanPath = $runtimePath -match '(?i)^(Private/AiExternalModelAcceleration\.ps1|Public/Get-SqlServerLabAiExternalModelPlan\.ps1|Schemas/ai-external-model-plan\.schema\.json|Tests/Static/Invoke-AiExternalModelAccelerationChecks\.ps1)$'
