...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiSharedGateway(Plan|Preflight|Registration|Storage)|Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGatewayPreflight|Register-SqlServerLabAiSharedGatewayStorage|ai-shared-gateway-(plan|preflight|storage))'; Checks = @('Invoke-AiSharedGatewayPlanChecks.ps1','Invoke-AiSharedGatewayPreflightChecks.ps1','Invoke-AiSharedGatewayStorageChecks.ps1','Invoke-AiExternalModelAccelerationChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiSharedGatewayPlanPath = $runtimePath -match '(?i)^(Private/AiSharedGateway(Plan|Preflight|Registration)\.ps1|Public/(Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGatewayPreflight|Register-SqlServerLabAiSharedGatewayStorage)\.ps1|Schemas/ai-shared-gateway-(plan|preflight-receipt|storage-(state|receipt))\.schema\.json|Tests/Static/Invoke-AiSharedGateway(Plan|Preflight|Storage)Checks\.ps1)$'
