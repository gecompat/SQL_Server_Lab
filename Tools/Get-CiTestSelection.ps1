...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiExternalModelAcceleration|AiExternalModelHyperVBinding|AiExternalModelPlan|AiExternalModelEndpoint|AiExternalModelArtifact|OvmsUpstream|OvmsHttpsGateway|ai-(external-model-(plan|endpoint|artifact|sql)|ovms-(upstream|https-gateway)))'; Checks = @('Invoke-AiExternalModelAccelerationChecks.ps1','Invoke-AiExternalModelHyperVBindingChecks.ps1') },
        @{ Pattern = '(?i)(AiComputeInventory|AiComputeCandidate|ai-compute-(inventory|candidate))'; Checks = @('Invoke-AiComputeInventoryChecks.ps1','Invoke-AiComputeSelectionChecks.ps1') },
        @{ Pattern = '(?i)(AiRuntimeCapability|ai-runtime-capability|LlamaCppRuntime)'; Checks = @('Invoke-AiRuntimeCapabilityChecks.ps1','Invoke-LlamaCppRuntimeChecks.ps1','Invoke-AiComputeInventoryChecks.ps1','Invoke-AiComputeSelectionChecks.ps1') },
        ...
        @{ Pattern = '(?i)(PublicCommandConsole|Invoke-PublicCommandConsoleChecks|ConsoleUi|ConsoleHelp|Invoke-SqlServerLab\.ps1|Start-SqlServerLabUi\.ps1|Ui[\\/]|Workflow)'; Checks = @('Invoke-PublicCommandConsoleChecks.ps1','Invoke-ConsoleUiChecks.ps1','Invoke-WorkflowUiChecks.ps1') },
        ...
        @{ Pattern = '(?i)(HyperVSqlStorageReconcile|hyperv-sql-storage-reconcile)'; Checks = @('Invoke-HyperVSqlStorageReconcileChecks.ps1','Invoke-HyperVSqlStorageReconcileAcceptanceChecks.ps1','Invoke-HyperVStorageReconcileChecks.ps1','Invoke-ReconcileActionContractChecks.ps1','Invoke-StorageFilePlacementChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiExternalModelPlanPath = $runtimePath -match '(?i)^(Private/(AiExternalModelAcceleration|AiOvmsHttpsGateway)\.ps1|Public/(Get-SqlServerLabAiExternalModelPlan|Test-SqlServerLabAiExternalModelEndpoint|Test-SqlServerLabAiExternalModelArtifact|Test-SqlServerLabOvmsUpstreamEndpoint|Start-SqlServerLabOvmsHttpsGateway|Stop-SqlServerLabOvmsHttpsGateway)\.ps1|Tools/Invoke-OvmsHttpsGatewayWorker\.ps1|Schemas/(ai-external-model-(plan|endpoint-receipt|artifact-receipt|sql-[a-z-]+)|ai-ovms-upstream-receipt)\.schema\.json|Tests/(Static/Invoke-AiExternalModel(Acceleration|HyperVBinding)Checks|Integration/Invoke-AiOvmsHttpsGatewayAcceptance)\.ps1)$'
            $aiSharedGatewayPlanPath = $runtimePath -match '(?i)^(Private/AiSharedGateway(Plan|Preflight|Registration|Upstream|Session|ServicePlan|ServiceSecret)\.ps1|Public/(Get-SqlServerLabAiSharedGateway(Plan|ServicePlan)|Test-SqlServerLabAiSharedGateway(Preflight|Upstream|ServiceSecret)|Register-SqlServerLabAiSharedGatewayStorage|(Start|Stop)-SqlServerLabAiSharedGatewaySession)\.ps1|Tools/Invoke-AiSharedGatewaySessionWorker\.ps1|Schemas/ai-shared-gateway-(plan|preflight-receipt|storage-(state|receipt)|upstream-receipt|session-receipt|service-plan|service-secret-receipt)\.schema\.json|Tests/Static/Invoke-AiSharedGateway(Plan|Preflight|Storage|Upstream|Session|ServicePlan|ServiceSecret)Checks\.ps1)$'
            $aiComputeBenchmarkPath = $runtimePath -match '(?i)^(Private/AiComputeBenchmark\.ps1|Public/Measure-SqlServerLabAiComputeCandidate(Set)?\.ps1|Schemas/ai-compute-benchmark(-set|-acceptance)?\.schema\.json|Tests/(Static/Invoke-AiComputeBenchmark(Acceptance)?Checks|Integration/Invoke-AiComputeBenchmarkAcceptance)\.ps1)$'
            if (-not $persistentRetrievalPath -and -not $aiPodmanSetupPath -and -not $aiPodmanSamplesReferencePath -and -not $sqlHttpsBridgePath -and -not $aiExternalModelPlanPath -and -not $aiSharedGatewayPlanPath -and -not $aiComputeBenchmarkPath -and $runtimePath -match '(?i)(^Private/Ai[^/]*\.ps1$|^Public/[^/]*SqlServerLabAi[^/]*\.ps1$|^Schemas/ai-[^/]*\.schema\.json$|SqlObservabilityEvidence|sql-observability-evidence)') { $pathRuntime.Docker = $true; $pathRuntime.Podman = $true; $pathRuntime.HyperV = $true }
            ...
            $staticOnlyProductChange = $staticOnlyProductChange -or $aiComputeBenchmarkPath
