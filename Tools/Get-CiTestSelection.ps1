...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiSharedGateway(Plan|Preflight|Registration|Storage|Upstream)|Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGateway(Preflight|Upstream)|Register-SqlServerLabAiSharedGatewayStorage|ai-shared-gateway-(plan|preflight|storage|upstream))'; Checks = @('Invoke-AiSharedGatewayPlanChecks.ps1','Invoke-AiSharedGatewayPreflightChecks.ps1','Invoke-AiSharedGatewayStorageChecks.ps1','Invoke-AiSharedGatewayUpstreamChecks.ps1','Invoke-AiExternalModelAccelerationChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiSharedGatewayPlanPath = $runtimePath -match '(?i)^(Private/AiSharedGateway(Plan|Preflight|Registration|Upstream)\.ps1|Public/(Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGateway(Preflight|Upstream)|Register-SqlServerLabAiSharedGatewayStorage)\.ps1|Schemas/ai-shared-gateway-(plan|preflight-receipt|storage-(state|receipt)|upstream-receipt)\.schema\.json|Tests/Static/Invoke-AiSharedGateway(Plan|Preflight|Storage|Upstream)Checks\.ps1)$'
