...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiSharedGateway(Plan|Preflight)|Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGatewayPreflight|ai-shared-gateway-(plan|preflight))'; Checks = @('Invoke-AiSharedGatewayPlanChecks.ps1','Invoke-AiSharedGatewayPreflightChecks.ps1','Invoke-AiExternalModelAccelerationChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiSharedGatewayPlanPath = $runtimePath -match '(?i)^(Private/AiSharedGateway(Plan|Preflight)\.ps1|Public/(Get-SqlServerLabAiSharedGatewayPlan|Test-SqlServerLabAiSharedGatewayPreflight)\.ps1|Schemas/ai-shared-gateway-(plan|preflight-receipt)\.schema\.json|Tests/Static/Invoke-AiSharedGateway(Plan|Preflight)Checks\.ps1)$'
            ...
            if (-not $persistentRetrievalPath -and -not $aiPodmanSetupPath -and -not $aiPodmanSamplesReferencePath -and -not $sqlHttpsBridgePath -and -not $aiExternalModelPlanPath -and -not $aiComputeBenchmarkPath -and $runtimePath -match '(?i)(^Private/Ai[^/]*\.ps1$|^Public/[^/]*SqlServerLabAi[^/]*\.ps1$|^Schemas/ai-[^/]*\.schema\.json$|SqlObservabilityEvidence|sql-observability-evidence)') { $pathRuntime.Docker = $true; $pathRuntime.Podman = $true; $pathRuntime.HyperV = $true }
