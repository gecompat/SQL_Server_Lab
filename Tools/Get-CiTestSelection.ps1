...
process {
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            if ($runtimePath -match '(?i)(HyperVSampleManifest|hyperv-sample-manifest|HyperV(Resource|Storage)Reconcile|hyperv-(resource|storage)-reconcile)') { $pathRuntime.HyperV = $true }
