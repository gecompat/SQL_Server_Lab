...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiComputeBenchmark|Measure-SqlServerLabAiComputeCandidate|ai-compute-benchmark)'; Checks = @('Invoke-AiComputeBenchmarkChecks.ps1','Invoke-AiComputeSelectionChecks.ps1','Invoke-AiRuntimeCapabilityChecks.ps1','Invoke-LlamaCppRuntimeChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiComputeBenchmarkPath = $runtimePath -match '(?i)^(Private/AiComputeBenchmark\.ps1|Public/Measure-SqlServerLabAiComputeCandidate\.ps1|Schemas/ai-compute-benchmark\.schema\.json|Tests/Static/Invoke-AiComputeBenchmarkChecks\.ps1)$'
