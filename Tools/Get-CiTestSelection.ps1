...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiExternalModelAcceleration|AiExternalModelPlan|AiExternalModelEndpoint|ai-external-model-(plan|endpoint))'; Checks = @('Invoke-AiExternalModelAccelerationChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiExternalModelPlanPath = $runtimePath -match '(?i)^(Private/AiExternalModelAcceleration\.ps1|Public/(Get-SqlServerLabAiExternalModelPlan|Test-SqlServerLabAiExternalModelEndpoint)\.ps1|Schemas/ai-external-model-(plan|endpoint-receipt)\.schema\.json|Tests/Static/Invoke-AiExternalModelAccelerationChecks\.ps1)$'
