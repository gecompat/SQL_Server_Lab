...
process {
    ...
    $staticGroups = @(
        ...
        @{ Pattern = '(?i)(AiSharedGatewayPlan|Get-SqlServerLabAiSharedGatewayPlan|ai-shared-gateway-plan)'; Checks = @('Invoke-AiSharedGatewayPlanChecks.ps1','Invoke-AiExternalModelAccelerationChecks.ps1') },
    ...
    if ($ciInfrastructure) {
        ...
        foreach ($runtimePath in $allPaths) {
            ...
            $aiSharedGatewayPlanPath = $runtimePath -match '(?i)^(Private/AiSharedGatewayPlan\.ps1|Public/Get-SqlServerLabAiSharedGatewayPlan\.ps1|Schemas/ai-shared-gateway-plan\.schema\.json|Tests/Static/Invoke-AiSharedGatewayPlanChecks\.ps1)$'
