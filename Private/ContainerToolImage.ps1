...
function Invoke-LabContainerToolImageBuild {
    ...
    return Invoke-LabArtifactStoreLock -StateRoot $StateRoot -ScriptBlock {
        ...
        try {
            ...
            $buildOperation = Invoke-LabProviderOperation -Provider $provider -Phase 'image-build' -StateRoot $StateRoot -NativeResult `
                -Command "$provider $(@($buildArguments) -join ' ')" -Action { Invoke-LabProgressNativeCommand -FilePath $runtimeInvocation -ArgumentList $buildArguments -Phase ImageBuild }
