...
function Complete-LabAiPersistentGeneration {
    ...
    if($MigrationJournal){
        ...
        $parameters.sourceRevision=Get-LabAiPersistentMigrationSourceRevision $MigrationJournal.migration
        $migrationGuard=@"
...
IF NOT EXISTS(SELECT 1 FROM [$name].dbo.LabGenerations WITH(UPDLOCK,HOLDLOCK) WHERE Generation=@previous AND Status='COMMITTED' AND PlanKey=@sourcePlan AND ModelHash=@sourceModel AND OperationId=@sourceOperation AND DatasetHash=@dataset AND Revision=@sourceRevision) THROW 51000,'AI_PERSISTENT_SOURCE_GENERATION_DRIFT',1;
IF (SELECT COUNT(*) FROM [$name].dbo.LabChunks WITH(UPDLOCK,HOLDLOCK) WHERE Generation=@previous)<>@documentCount OR EXISTS(SELECT ChunkId COLLATE Latin1_General_100_BIN2,ChunkSha256,SourceVectorSha256 FROM OPENJSON(@receipt,'$.sourceChunks') WITH(ChunkId nvarchar(96),ChunkSha256 char(64),SourceVectorSha256 char(64)) EXCEPT SELECT ChunkId,ContentHash,LOWER(CONVERT(char(64),HASHBYTES('SHA2_256',CAST(Embedding AS varchar(max))),2)) FROM [$name].dbo.LabChunks WHERE Generation=@previous) OR EXISTS(SELECT Id COLLATE Latin1_General_100_BIN2,DATALENGTH(Id),Content COLLATE Latin1_General_100_BIN2,DATALENGTH(Content),ContentHash FROM OPENJSON(@manifest) WITH(Id nvarchar(96),Content nvarchar(4000),ContentHash char(64)) EXCEPT SELECT ChunkId,DATALENGTH(ChunkId),Content COLLATE Latin1_General_100_BIN2,DATALENGTH(Content),ContentHash FROM [$name].dbo.LabChunks WHERE Generation=@previous) THROW 51000,'AI_PERSISTENT_SOURCE_GENERATION_DRIFT',1;
"@
    }
