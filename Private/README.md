...
| `AiSharedGatewayServiceSecret.ps1` | Read-only SecretManagement-Auflösung aller Consumer-Referenzen des aktuellen Principals mit Typ-/Formatprüfung und sanitisiertem Receipt; echter Dienst-Logon bleibt offene Evidence |
...
| `PersistentStorageCatalog.ps1` | Parser und read-only Planner für stabile Persistent-Storage-IDs, Klassen, Zustände, Referenzen, exklusive Leases und Registrierungskandidaten; generischer CAS-/Preview-Mutationskern, gemeinsamer Artifact-Writer sowie atomare Backup-, Paket-, sichere Exchange-Workspace- und reguläre Hyper-V-Instanzstore-Reservierungswriter |
| `PersistentStorageRecovery.ps1` | Original-Evidence-, Ownership- und Runtime-Bindung der engen retained Containerstore-Katalog-Recovery sowie gemeinsame Consumer-Guards |
