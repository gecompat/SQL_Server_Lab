...
function Get-LabPortableContainerTransferPreflightBackups {
    ...
    foreach($transfer in @($Request.DatabaseTransfers)){
        ...
        try{
            ...
            $expectedRelative=('Objects/'+([string]$record.Artifact.Sha256).ToLowerInvariant()+'.bak');if([string]$record.Artifact.RelativePath -cne $expectedRelative){throw 'BACKUP_LIBRARY_OBJECT_PATH_NOT_CANONICAL'}
...
function Get-LabPortableContainerTransferPreflightTargetBinding {
    ...
    try{
        $run=Get-LabRunState -RunId ([string]$Request.TargetRunId) -StateRoot $StateRoot;$target=Resolve-LabRunInstance -RunId ([string]$Request.TargetRunId) -InstanceId ([string]$Request.TargetInstanceId) -StateRoot $StateRoot
        if([string]$run.state -ne 'RUNNING' -or [string]$target.Provider -notin @('docker','podman')){throw 'binding'};$public.Provider=[string]$target.Provider
        $scope=Get-LabContainerRuntimeScope -Provider ([string]$target.Provider);if([string]$scope.Status -ne 'AVAILABLE' -or [string]::IsNullOrWhiteSpace([string]$scope.RuntimeId)){throw 'binding'};$public.RuntimeScopeId=[string]$scope.RuntimeId
        $runtime=Get-LabHostToolInvocation -Name ([string]$target.Provider);$inspection=@(& $runtime inspect ([string]$target.ContainerName) 2>$null);if($LASTEXITCODE -ne 0 -or $inspection.Count -eq 0){throw 'binding'};$container=($inspection -join "`n")|ConvertFrom-Json -Depth 30;if($container -is [array]){if($container.Count -ne 1){throw 'binding'};$container=$container[0]}
        $containerId=[string]$container.Id;if(-not[bool]$container.State.Running -or $containerId -notmatch '^[0-9a-fA-F]{64}$'){throw 'binding'}
        $labels=$container.Config.Labels;if([string]$labels.'sql-server-lab.run-id' -cne [string]$Request.TargetRunId -or [string]$labels.'sql-server-lab.scope-id' -cne [string]$run.scopeId -or [string]$labels.'sql-server-lab.instance-id' -cne [string]$Request.TargetInstanceId){throw 'binding'}
...
function Get-LabPortableContainerTransferPreflightTargetBinding {
    ...
    try{
        ...
        $containerId=[string]$container.Id;if(-not[bool]$container.State.Running -or $containerId -notmatch '^[0-9a-fA-F]{64}$'){throw 'binding'}
        $labels=$container.Config.Labels;if([string]$labels.'sql-server-lab.run-id' -cne [string]$Request.TargetRunId -or [string]$labels.'sql-server-lab.scope-id' -cne [string]$run.scopeId -or [string]$labels.'sql-server-lab.instance-id' -cne [string]$Request.TargetInstanceId){throw 'binding'}
        $mount=@($container.Mounts|Where-Object{$_ -and [string]$_.Type -eq 'bind' -and [string]$_.Destination -eq '/var/opt/mssql/backup' -and [bool]$_.RW});if($mount.Count -ne 1){throw 'binding'};$mounted=(Resolve-Path -LiteralPath ([string]$mount[0].Source) -ErrorAction Stop).Path;if(-not[string]::Equals($hostRoot,$mounted,[StringComparison]::OrdinalIgnoreCase)){throw 'binding'}
        if([string]::IsNullOrWhiteSpace([string]$target.HostName) -or [int]$target.Port -lt 1){throw 'binding'};$ports=@($container.NetworkSettings.Ports.'1433/tcp');if($ports.Count -ne 1 -or [int]$ports[0].HostPort -ne [int]$target.Port -or [string]$ports[0].HostIp -notin @('127.0.0.1','::1')){throw 'binding'}
        $public.BindingStatus='VERIFIED';[PSCustomObject]@{Available=$true;Code=$null;Public=[PSCustomObject]$public;HostRoot=$hostRoot;HostName=[string]$target.HostName;Port=[int]$target.Port;ContainerId=$containerId.ToLowerInvariant()}
    }catch{[PSCustomObject]@{Available=$false;Code='SQL_VISIBLE_STAGING_BINDING_UNAVAILABLE';Public=[PSCustomObject]$public}}
}
...
.Destination -eq '/var/opt/mssql/backup' -and [bool]<#
.SYNOPSIS
    Prueft explizit ausgewaehlte Backupsets im bereits gebundenen SQL-Backup-Mount vor.
.DESCRIPTION
    Dieser Vertrag ist kein Transferexecutor. Er akzeptiert nur explizite DatabaseTransfers,
...
.RW});if($mount.Count -ne 1 -or [string]::IsNullOrWhiteSpace([string]$mount[0].Source)){throw 'binding'};$hostRoot=(Resolve-Path -LiteralPath ([string]$mount[0].Source) -ErrorAction Stop).Path;Assert-LabPortableContainerTransferPreflightNoReparsePath -Root (Split-Path -Parent $hostRoot) -Path $hostRoot
        if([string]::IsNullOrWhiteSpace([string]$target.HostName) -or [int]$target.Port -lt 1){throw 'binding'};$ports=@($container.NetworkSettings.Ports.'1433/tcp');if($ports.Count -ne 1 -or [int]$ports[0].HostPort -ne [int]$target.Port -or [string]$ports[0].HostIp -notin @('127.0.0.1','::1')){throw 'binding'}
        $public.BindingStatus='VERIFIED';[PSCustomObject]@{Available=$true;Code=$null;Public=[PSCustomObject]$public;HostRoot=$hostRoot;HostName=[string]$target.HostName;Port=[int]$target.Port;ContainerId=$containerId.ToLowerInvariant()}
    }catch{[PSCustomObject]@{Available=$false;Code='SQL_VISIBLE_STAGING_BINDING_UNAVAILABLE';Public=[PSCustomObject]$public}}
}
