/* QRY-001 cleanup: drop only after complete FWK-002 marker verification. */
USE [master];
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF @DemoId<>'QRY-001' OR @TargetDatabase<>@ExpectedDatabase
    THROW 51000, 'FAIL_CONTRACT: QRY-001-Cleanupziel ist ungültig.', 1;
...
SELECT 1 AS Sequence,'CLEANUP' AS Phase,'SUMMARY' AS CheckId,'PASS' AS Outcome,'OK' AS Code,
       N'markierte QRY-001-Datenbank entfernt' AS ObservedValue,N'vollständige Markerübereinstimmung' AS RequiredValue,
