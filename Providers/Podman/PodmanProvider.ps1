...
function Initialize-PodmanSqlNamedVolume {
    ...
    if ($volumeExists -and $PersistentStorageId) {
        ...
        catch { throw "PODMAN_SQL_VOLUME_INSPECT_INVALID: $VolumeName" }
            ...
            while ($true) {
                ...
                if (-not $automaticPort) {
                    ...
                    $ignoredBackup = $EndpointBindingIgnoreContainerName -and
                        ([string]$binding.Owner).StartsWith("podman:$EndpointBindingIgnoreContainerName (", [StringComparison]::Ordinal)
