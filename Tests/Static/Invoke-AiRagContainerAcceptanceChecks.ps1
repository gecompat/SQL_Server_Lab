...
try {
    ...
    $script:elapsed=0;$script:pullAttempts=0
    Check 'Auch eine verspätete Erfolgsantwort überschreitet das gemeinsame Pullbudget' ((Reject {Invoke-AiRagModelPull -ModelRole GENERATION -Model 'synthetic' -Port 11434 -TimeoutSeconds 60 -Request {param($Uri,$Body,$Timeout)$script:pullAttempts++;$script:elapsed=61000;[pscustomobject]@{status='success'}} -ElapsedMilliseconds {$script:elapsed} -Delay {throw 'unexpected retry'}} 'AI_RAG_MODEL_PULL_TIMEOUT') -and $script:pullAttempts -eq 1)
