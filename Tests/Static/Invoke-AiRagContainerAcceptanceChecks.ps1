...
try {
    ...
    $embedding=Get-AiRagFailureReceipt -Provider podman -Phase FIRST_RAG -ErrorRecord ([pscustomobject]@{Exception=[Exception]::new('AI_ENDPOINT_TIMEOUT');ScriptStackTrace='at Invoke-LabAiRag, AiRag.ps1: line 97'})
    $generation=Get-AiRagFailureReceipt -Provider podman -Phase RESTART_RAG -ErrorRecord ([pscustomobject]@{Exception=[Exception]::new('AI_ENDPOINT_TIMEOUT');ScriptStackTrace='at Invoke-LabAiRag, AiRag.ps1: line 124'})
