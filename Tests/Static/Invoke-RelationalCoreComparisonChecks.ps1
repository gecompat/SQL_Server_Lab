...
$characterization=& $module {
    ...
    $unsupported=[PSCustomObject]@{HasRls=$false;HasPrimaryKey=$true;PrimaryKeyOrder='[id]';SelectProjection='[id]';UnsupportedReasons='UNSUPPORTED'}
    ...
    [PSCustomObject]@{NullDistinct=($nullBytes -join ',') -ne ($emptyBytes -join ',');EmptyDistinctFromSpace=($emptyBytes -join ',') -ne ($spaceBytes -join ',');UnicodeExact=($unicodeBytes -join ',') -eq '1,195,164';BinaryExact=($binaryBytes -join ',') -eq '1,0,255';DecimalBlocked=$decimalBlocked;UnsupportedBlocked=-not (Test-LabRelationalCoreTableSupported -Table $unsupported);MissingReadOnlyBlocked=$missingReadOnly;WrongIdentityBlocked=$wrongIdentity}
}
...
Add-RelationalCoreCheck 'Charakterisierung blockiert Decimal, unsichere Spalten und fehlendes READ_ONLY' ($characterization.DecimalBlocked -and $characterization.UnsupportedBlocked -and $characterization.MissingReadOnlyBlocked -and $characterization.WrongIdentityBlocked)
