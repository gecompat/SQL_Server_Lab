...
Add-RelationalCoreCheck 'Allowlist und RLS-/Sondertabellen-Blockade sind fail-closed' ($private -match 'HasRls' -and $private -match 't\.temporal_type=0' -and $private -match 't\.is_filetable=0' -and $private -match 't\.is_external=0' -and $private -match 't\.is_memory_optimized=0' -and $private -match "N'datetime2'" -and $private -notmatch "N'money'" -and $private -notmatch "N'smalldatetime'" -and $private -notmatch "N'decimal'" -and $private -match 'TABLE_UNSUPPORTED_OR_POLICY_BLOCKED')
...
Add-RelationalCoreCheck 'Charakterisierung blockiert Decimal, unzulässige PKs, max/versteckte Spalten und fehlendes READ_ONLY' ($characterization.DecimalBlocked -and $characterization.AllowedTable -and $characterization.UnsupportedPkBlocked -and $characterization.MaxColumnBlocked -and $characterization.HiddenColumnBlocked -and $characterization.UnsupportedTableResultCodes -and $characterization.MissingReadOnlyBlocked -and $characterization.WrongIdentityBlocked)
