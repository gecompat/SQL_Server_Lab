...
try {
    ...
    Add-CheckResult `
        ...
        -Success ($metadata.runtimeStatus -eq 'windows-specialization-sql-readiness-orchestration' -and $metadata.sqlProvisioning -eq $false -and $metadata.sqlProvisioningScope -eq 'prepared-image-clone-only' -and $metadata.limitations -notcontains 'no-real-windows-sql-e2e-evidence' -and $metadata.limitations -notcontains 'no-sql-complete-image-runtime' -and $metadata.capabilities -contains 'powershell-direct-software-installation' -and $metadata.capabilities -contains 'sql-external-runtime' -and $metadata.capabilities -contains 'hyperv-resource-reconcile' -and $metadata.capabilities -contains 'hyperv-storage-reconcile')
    Add-CheckResult `
