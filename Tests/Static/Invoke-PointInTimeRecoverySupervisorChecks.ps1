...
try {
    ...
    $record=@{OperationId=$operation;Provider='docker';StateRoot=$state}
    Assert-PitrChildBinding -Record $record -Provider docker -OperationId $operation -StateRoot $state -EvidenceRoot $root
    Check $true 'Actual child accepts its exact parent-selected operation, provider and isolated StateRoot'
    foreach ($field in @('OperationId','Provider','StateRoot')) {
        $changed=$record.Clone(); $changed[$field]='foreign'
        $caught=$null
        try { Assert-PitrChildBinding -Record $changed -Provider docker -OperationId $operation -StateRoot $state -EvidenceRoot $root }
        catch { $caught=$_.Exception.Message }
        Check ($caught -ceq 'PITR_CHILD_BINDING_INVALID') "Actual child rejects changed $field before any provider access"
    }
    $pending=[Threading.Tasks.TaskCompletionSource[bool]]::new()
    Check (-not (Wait-PitrOutputDrain -Task @($pending.Task) -TimeoutMilliseconds 1)) 'Incomplete output draining fails closed'
    $pending.SetException([InvalidOperationException]::new('synthetic drain failure'))
    Check (-not (Wait-PitrOutputDrain -Task @($pending.Task) -TimeoutMilliseconds 1)) 'Faulted output draining fails closed'
    Check (Wait-PitrOutputDrain -Task @([Threading.Tasks.Task]::CompletedTask)) 'Completed output draining succeeds'
...
'@ | Set-Content -LiteralPath $runner
    foreach ($mode in @('success','failed','partial','timeout','cancelled','cleanup-failed')) {
        ...
        try {
            $output=@(Invoke-PitrSupervisor -AcceptanceRunner $runner -Provider 'synthetic' `
    ...
    $script:childCalls=0
    function Invoke-PitrSupervisedChild {
        param($Control)
    ...
    $blocked=Invoke-PitrSupervisor -AcceptanceRunner $runner -Provider 'synthetic' `
    ...
    Write-Host "PITR SUPERVISOR: $count PASS"
}
finally {
    $resolved=[IO.Path]::GetFullPath($root)
    $temp=[IO.Path]::GetFullPath([IO.Path]::GetTempPath())
    if (-not $resolved.StartsWith($temp,[StringComparison]::OrdinalIgnoreCase) -or
        [IO.Path]::GetFileName($resolved) -notlike 'sql-server-lab-pitr-*') { throw 'SYNTHETIC_ROOT_INVALID' }
