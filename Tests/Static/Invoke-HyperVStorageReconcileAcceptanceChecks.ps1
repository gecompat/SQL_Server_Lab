...
$checks=[ordered]@{
 ...
 'Manifest-bound SCSI additions have exact host and guest postconditions without destructive shrink simulation'=($source -match "id='data'" -and $source -match 'ControllerLocation' -and $source -notmatch 'Resize-VHD' -and $source -match 'guestDriveInitialization' -and $source -match 'GUEST_VERIFIED')
