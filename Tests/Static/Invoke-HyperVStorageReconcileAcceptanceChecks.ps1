...
$checks=[ordered]@{
 ...
 'Manifest-bound SCSI additions use schema-valid drive mappings with exact host and guest postconditions without destructive shrink simulation'=($source -match "id='data';containerPath='E:\\SQLData';sizeLimitGB=4" -and $source -match "id='log';containerPath='L:\\SQLLog';sizeLimitGB=2" -and $source -match 'ControllerLocation' -and $source -notmatch 'Resize-VHD' -and $source -match 'guestDriveInitialization' -and $source -match 'GUEST_VERIFIED')
