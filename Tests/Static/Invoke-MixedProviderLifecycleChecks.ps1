...
if ($failures.Count -eq 0) {
    ...
    Assert-Contains $getLab '\$provider -notin @\(''docker'', ''podman''\)' 'Get-SqlServerLab behandelt Hyper-V weiterhin wie eine Container-Runtime.'
    Assert-Contains $getLab 'Get-HyperVInstanceStatus' 'Get-SqlServerLab fragt keinen providergerechten Hyper-V-VM-Status ab.'
    Assert-Contains $getLab 'RuntimeState' 'Get-SqlServerLab liefert keinen providerneutralen Runtime-State.'
    Assert-Contains $getLab '\$instance\.sqlVersion' 'Get-SqlServerLab verliert die SQL-Version nativer Hyper-V-Instanzen.'
    Assert-Contains $getLab 'SQL nicht live geprueft' 'Get-SqlServerLab trennt VM-Laufzustand und ungepruefte SQL-Bereitschaft nicht.'
