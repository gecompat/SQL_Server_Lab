...
if ($failures.Count -eq 0) {
    ...
    Assert-Contains $sqlReadiness '&\s+\$podmanInvocation\s+machine\s+list\s+--format\s+json' 'Podman-Windows-Hostauflösung ermittelt die aktive Machine nicht strukturiert.'
    ...
    Assert-Contains $containerReconcile 'RepairSqlRuntimeContract' 'Container-Reconcile kann den SQL-Runtime-Vertrag nicht gezielt erneuern.'
    Assert-Contains $containerReconcile 'MSSQL_MEMORY_LIMIT_MB=\$sqlMemoryLimitMB' 'Container-Reconcile erneuert das SQL-interne Memory-Limit nicht.'
    Assert-Contains $containerReconcile '--health-cmd[^\r\n]+\s-C\s' 'Container-Reconcile erneuert keinen TLS-tauglichen Healthcheck.'
    Assert-Contains $containerReconcile 'NetworkSettings\.Networks' 'Container-Reconcile erhält die gebundene Runtime-Netzwerkzuordnung nicht.'
