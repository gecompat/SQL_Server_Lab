...
try{
    ...
    $checks=[ordered]@{
        ...
        'Nativer Runner bindet Plan, WhatIf, Add, No-op, bereinigtes Remove-Journal und VM-Restart an die oeffentliche CLI'=($acceptanceSource -match 'Get-SqlServerLabReconcilePlan' -and $acceptanceSource -match 'Invoke-SqlServerLabReconcileAction' -and $acceptanceSource -match 'add-sample' -and $acceptanceSource -match 'remove-owned-sample' -and $acceptanceSource -match 'removeResult\.JournalStatus' -and $acceptanceSource -match '-not \(Test-Path -LiteralPath \$journalPath\)' -and $acceptanceSource -match 'Restart-SqlServerLab')
