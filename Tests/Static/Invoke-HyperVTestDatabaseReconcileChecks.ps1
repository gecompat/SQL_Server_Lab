...
try{
    ...
    $checks=[ordered]@{
        ...
        'Nativer Runner initialisiert seinen isolierten Storage-Default, erzeugt, verifiziert und verwendet LAB_GENERATED erneut'=($acceptanceSource -match 'SQL_SERVER_LAB_TEST_DATA_ROOT' -and $acceptanceSource -match 'Initialize-LabManagedDataRoot\s+-DataRoot \$Root' -and $acceptanceSource -match 'Get-LabSampleBaselineRequest' -and $acceptanceSource -match 'Get-FileHash' -and $acceptanceSource -match "resolvedArtifact\.origin -eq 'LAB_GENERATED'" -and $acceptanceSource -match 'baselineAddResult' -and $acceptanceSource -match 'baselineRemoveResult')
