...
try{
    $result=&$module{
        ...
        $emptyTargetHash=Get-LabHyperVTestDatabaseTargetHash -Samples @()
        ...
        [PSCustomObject]@{
            Intent=$intentValid;EmptyTargetHash=($emptyTargetHash -match '^[a-f0-9]{64}$');Plan=$plan.HighestChangeClass -eq 'live' -and @($plan.Diff.Kind) -contains 'add-sample' -and @($plan.Diff.Kind) -contains 'remove-owned-sample'
            WhatIf=$whatIfSafe -and $whatIf.ExecutionSummary.Status -eq 'WOULD_EXECUTE'
            Apply=$applied.ExecutionSummary.Status -eq 'SUCCEEDED' -and $applied.ExecutionPlan[0].Result.JournalStatus -eq 'COMPLETED' -and $journal.Status -eq 'COMPLETED' -and $script:dbRemoveCount -ge 1 -and $script:dbInstallCount -ge 1 -and $script:dbSyncCount -ge 1 -and $privatePreserved
    ...
    $checks=[ordered]@{
        ...
        'Leeres Ziel-Sample-Set besitzt einen stabilen TargetHash fuer eigentumsgebundene Entfernungen'=$result.EmptyTargetHash
        ...
        'Nativer Runner bindet Plan, WhatIf, Add, No-op, abgeschlossenes Journal, VM-Restart und stabile Host-SQL-Readiness an die oeffentliche CLI'=($acceptanceSource -match 'Get-SqlServerLabReconcilePlan' -and $acceptanceSource -match 'Invoke-SqlServerLabReconcileAction' -and $acceptanceSource -match 'add-sample' -and $acceptanceSource -match 'remove-owned-sample' -and $acceptanceSource -match 'removeResult\.ExecutionPlan\[0\]\.Result\.JournalStatus' -and $acceptanceSource -match '\$completedJournalSchemaValid' -and $acceptanceSource -match '\(Test-Path -LiteralPath \$journalPath\)' -and $acceptanceSource -match 'Restart-SqlServerLab' -and $acceptanceSource -match 'Wait-AcceptanceSqlReady' -and $acceptanceSource -match 'Wait-SqlReady')
        ...
        'Nativer Runner initialisiert seine isolierte Sample-Bibliothek, bindet und prueft den Artifact-Storage, deklariert temporaeren Aktivierungs-Egress und verwendet LAB_GENERATED erneut'=($acceptanceSource -match 'SQL_SERVER_LAB_TEST_DATA_ROOT' -and $acceptanceSource -match 'Initialize-LabManagedDataRoot\s+-DataRoot \$Root' -and $acceptanceSource -notmatch 'Register-LabDataRoot' -and $acceptanceSource -match 'Get-HyperVArtifactStorageConfiguration\s+-Artifact \$Artifact' -and $acceptanceSource -match 'Artifact-gebundener Storage-Default ist vor der Provisionierung aufloesbar' -and $acceptanceSource -match 'New-LabStorageBoundPlan\s+-StorageIntent \$Intent' -and $acceptanceSource -match '-StorageConfiguration \$StorageConfiguration' -and $acceptanceSource -match "ContractVersion='SqlServerLab.WindowsActivationIntent/1.0'" -and $acceptanceSource -match "EgressPolicy='AllowTemporary'" -and $acceptanceSource -match '\$script:saPassword\.MakeReadOnly\(\)' -and $acceptanceSource -match 'previousDataRoot' -and $acceptanceSource -match 'previousControllerId' -and $acceptanceSource -match 'Get-LabSampleBaselineRequest' -and $acceptanceSource -match 'Get-FileHash' -and $acceptanceSource -match "resolvedArtifact\.origin -eq 'LAB_GENERATED'" -and $acceptanceSource -match 'baselineAddResult' -and $acceptanceSource -match 'baselineRemoveResult')
