...
try{
    ...
    $checks=[ordered]@{
        ...
        'Nativer Runner bindet Plan, WhatIf, Add, No-op, abgeschlossenes Journal und VM-Restart an die oeffentliche CLI'=($acceptanceSource -match 'Get-SqlServerLabReconcilePlan' -and $acceptanceSource -match 'Invoke-SqlServerLabReconcileAction' -and $acceptanceSource -match 'add-sample' -and $acceptanceSource -match 'remove-owned-sample' -and $acceptanceSource -match 'removeResult\.ExecutionPlan\[0\]\.Result\.JournalStatus' -and $acceptanceSource -match '\$completedJournalSchemaValid' -and $acceptanceSource -match '\(Test-Path -LiteralPath \$journalPath\)' -and $acceptanceSource -match 'Restart-SqlServerLab')
