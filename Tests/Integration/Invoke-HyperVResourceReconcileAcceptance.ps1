...
.DESCRIPTION
    ...
    eigentumsgebundene oeffentliche Reparatur. Anschliessend injiziert der
    Runner nur fuer seine eigene statische VM einen einmaligen Fehler direkt
    vor Start-VM, entfernt den test-only Modulwrapper und setzt denselben
    Executor fort. Dies ist keine Hyper-V-Plattformfehlersimulation.
#>
[CmdletBinding()]
param(
    [string]$ArtifactId,
...
try {
    ...
    $startVmCommand=Get-Command Start-VM -ErrorAction Stop
    Assert-HyperVResourceAcceptance ($startVmCommand.CommandType -eq 'Cmdlet') 'Start-VM ist vor der Fault-Injection nicht umgeleitet'
    if(-not (Test-Path -LiteralPath $faultSupportPath -PathType Leaf)){throw 'HYPERV_RESOURCE_ACCEPTANCE_FAULT_SUPPORT_MISSING'}
    . $faultSupportPath
    ...
    $stage='STATIC_FAULT_DRIFT';
    $staticVm=(Get-OwnedVm $staticContext).VM;Stop-VM -VM $staticVm -Confirm:$false -ErrorAction Stop
    $deadline=[datetime]::UtcNow.AddMinutes(3);do{Start-Sleep -Seconds 2;$staticVm=(Get-OwnedVm $staticContext).VM}while([string]$staticVm.State -ne 'Off' -and [datetime]::UtcNow -lt $deadline);Assert-HyperVResourceAcceptance ([string]$staticVm.State -eq 'Off') 'Run-eigene VM ist fuer die Fault-Resume-Drift gestoppt'
    Set-VMProcessor -VM $staticVm -Count 2 -ErrorAction Stop;Set-VMMemory -VM $staticVm -DynamicMemoryEnabled $true -MinimumBytes 1024MB -StartupBytes 4096MB -MaximumBytes 8192MB -ErrorAction Stop;Start-VM -VM $staticVm -ErrorAction Stop
    $deadline=[datetime]::UtcNow.AddMinutes(5);do{Start-Sleep -Seconds 3;$faultDriftValues=Get-ResourceValues $staticContext}while([string]$faultDriftValues.State -ne 'Running' -and [datetime]::UtcNow -lt $deadline);Assert-HyperVResourceAcceptance ($faultDriftValues.Cpu -eq 2 -and $faultDriftValues.Dynamic -and $faultDriftValues.Startup -eq 4096) 'Fault-Resume-Drift ist nur fuer die operationseigene VM hergestellt'
    $stage='STATIC_FAULT_SQL_READINESS';
    Assert-HyperVResourceAcceptance (Wait-ResourceSqlReady $staticContext $sa) 'SQL ist vor der injizierten Restart-Unterbrechung bereit'
    $stage='STATIC_FAULT_SHUTDOWN_READINESS';
    Assert-HyperVResourceAcceptance (Wait-ResourceShutdownIntegrationReady $staticContext) 'Hyper-V-Shutdown-Integration ist vor der injizierten Restart-Unterbrechung bereit'
    $stage='STATIC_FAULT_INJECT';
    $faultTarget=New-HyperVResourceAcceptanceStartVmFaultTarget -RunId ([string]$staticContext.Run.runId) -ScopeId ([string]$staticContext.Run.scopeId) -InstanceId ([string]$staticContext.Instance.id) -VMId ([string]$faultDriftValues.Id)
    $originalStartVm=({param($VM)& $startVmCommand -VM $VM -ErrorAction Stop}.GetNewClosure())
    $faultWrapperInstalled=$false
    try {
        $null=Install-HyperVResourceAcceptanceStartVmFaultWrapper -Module $module -Target $faultTarget -ExpectedRunId ([string]$staticContext.Run.runId) -ExpectedScopeId ([string]$staticContext.Run.scopeId) -ExpectedInstanceId ([string]$staticContext.Instance.id) -ExpectedVMId ([string]$faultDriftValues.Id) -OriginalStartVm $originalStartVm
        $faultWrapperInstalled=$true
        $faultResult=Invoke-SqlServerLabReconcileAction -RunId $staticLab.RunId -RepairHyperVResources -InstanceId primary -StateRoot $StateRoot -Confirm:$false
    } finally {
        if($faultWrapperInstalled){Remove-HyperVResourceAcceptanceStartVmFaultWrapper -Module $module}
    }
    $faultReceipt=Get-Content -LiteralPath $staticJournal -Raw -Encoding utf8|ConvertFrom-Json -Depth 30;$faultValues=Get-ResourceValues $staticContext;$faultWrapperState=Get-HyperVResourceAcceptanceStartVmFaultWrapperState -Module $module
    Assert-HyperVResourceAcceptance ([string]$faultResult.ExecutionSummary.Status -eq 'FAILED' -and @($faultResult.ExecutionSummary.Errors) -match 'HYPERV_RESOURCE_RECONCILE_ACCEPTANCE_INJECTED_PRESTART_FAILURE' -and [string]$faultReceipt.Status -eq 'RECOVERY_REQUIRED' -and [string]$faultReceipt.Recovery.ErrorCode -eq 'HYPERV_RESOURCE_RECONCILE_ACCEPTANCE_INJECTED_PRESTART_FAILURE' -and $faultTarget.Used -and -not $faultTarget.Armed -and -not $faultWrapperState.WrapperPresent -and [string]$faultValues.State -eq 'Off' -and $faultValues.Cpu -eq 4 -and -not $faultValues.Dynamic -and $faultValues.Startup -eq 6144) 'Injizierter Pre-Start-Fehler bleibt VM-ID-gebunden als Recovery sichtbar und entfernt den Wrapper'
    $stage='STATIC_RESUME';
    $resumeResult=Invoke-SqlServerLabReconcileAction -RunId $staticLab.RunId -RepairHyperVResources -InstanceId primary -StateRoot $StateRoot -Confirm:$false;$resumeValues=Get-ResourceValues $staticContext;$resumeReceipt=Get-Content -LiteralPath $staticJournal -Raw -Encoding utf8|ConvertFrom-Json -Depth 30
    $resumeMarker=Wait-ResourcePersistentSqlMarker $staticContext $sa
    Assert-HyperVResourceAcceptance ([string]$resumeResult.ExecutionSummary.Status -eq 'SUCCEEDED' -and [string]$resumeReceipt.Status -eq 'COMPLETED' -and [string]$resumeValues.Id -eq [string]$faultTarget.VMId -and [string]$resumeValues.State -eq 'Running' -and $resumeValues.Cpu -eq 4 -and -not $resumeValues.Dynamic -and $resumeValues.Startup -eq 6144 -and $resumeMarker -eq '1') 'Originales Start-VM und derselbe Executor setzen den journalisierten Restart idempotent fort'
    $stage='STATIC_NOOP';
