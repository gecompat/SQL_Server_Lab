...
try {
    ...
    Assert-HyperVSqlStorageAcceptance ([string]$result.ExecutionSummary.Status -eq 'SUCCEEDED' -and [string]$result.ExecutionPlan[0].Result.ReceiptStatus -eq 'VERIFIED' -and [string]$receiptAfter.Status -eq 'VERIFIED' -and $defaultsOk -and $tempOk -and $after.Defaults.SqlStartTime -ne $sqlStartBefore -and $bootAfter -eq $bootBefore -and [string]((Get-OwnedManagedVm $context).VM.State) -eq 'Running') 'Receipt, SQL-Defaults und TempDB konvergieren nach SQL-Dienstrestart ohne VM-Neustart'
