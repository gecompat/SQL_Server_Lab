...
try {
    ...
    Assert-HyperVSqlStorageAcceptance ([string]$hostPlan.HighestChangeClass -eq 'live' -and @($hostPlan.Diff.Kind|Sort-Object -Unique) -join ',' -eq 'add') 'HV-603 plant nur die manifestgebundenen SCSI-Additionen'
    ...
    $expected=@($receiptAfter.FileBindings);$defaultsOk=(@($expected|Where-Object Role -eq 'default-data')[0].SqlPhysicalPath -eq $after.Defaults.DefaultData -and @($expected|Where-Object Role -eq 'default-log')[0].SqlPhysicalPath -eq $after.Defaults.DefaultLog -and @($expected|Where-Object Role -eq 'backup')[0].SqlPhysicalPath -eq $after.Defaults.BackupDirectory)
    $tempOk=@($expected|Where-Object Role -in @('tempdb-data','tempdb-log')|Where-Object{$expectedFile=$_;$actual=@($after.TempDb|Where-Object LogicalName -eq [string]$expectedFile.LogicalName);$actual.Count -eq 1 -and [string]$actual[0].SqlPhysicalPath -eq [string]$expectedFile.SqlPhysicalPath}).Count -eq @($expected|Where-Object Role -in @('tempdb-data','tempdb-log')).Count
    Assert-HyperVSqlStorageAcceptance ([string]$result.ExecutionSummary.Status -eq 'SUCCEEDED' -and [string]$result.Actions[0].Result.ReceiptStatus -eq 'VERIFIED' -and [string]$receiptAfter.Status -eq 'VERIFIED' -and $defaultsOk -and $tempOk -and $after.Defaults.SqlStartTime -ne $sqlStartBefore -and $bootAfter -eq $bootBefore -and [string]((Get-OwnedManagedVm $context).VM.State) -eq 'Running') 'Receipt, SQL-Defaults und TempDB konvergieren nach SQL-Dienstrestart ohne VM-Neustart'
