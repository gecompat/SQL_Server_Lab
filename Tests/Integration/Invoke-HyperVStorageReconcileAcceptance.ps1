...
try {
    ...
    $managedSlots=@($initial|ForEach-Object{[int]$_.controllerLocation});$managedAttachments=@($attachments|Where-Object{[string]$_.Path -in @($initial.path) -and [int]$_.ControllerNumber -eq 0})
    $slotBindings=@($initial|ForEach-Object{$drive=$_;@($managedAttachments|Where-Object{[string]::Equals([string]$_.Path,[string]$drive.path,[StringComparison]::OrdinalIgnoreCase) -and [int]$_.ControllerLocation -eq [int]$drive.controllerLocation}).Count -eq 1})
    Assert-HyperVStorageAcceptance ($initial.Count -eq 2 -and [long]$data.sizeBytes -eq 4GB -and [long]$log.sizeBytes -eq 2GB -and $managedSlots.Count -eq 2 -and @($managedSlots|Sort-Object -Unique).Count -eq 2 -and @($managedSlots|Where-Object{$_ -lt 1}).Count -eq 0 -and $managedAttachments.Count -eq 2 -and @($slotBindings|Where-Object{$_}).Count -eq 2 -and @($initial|Where-Object{[string]$_.diskIdentifier -notmatch '^[A-Fa-f0-9-]{36}$'}).Count -eq 0) 'Manifestgebundene SCSI-Additionen besitzen Host-VHDX, freie eindeutige Slots und Disk-Identitaeten'
