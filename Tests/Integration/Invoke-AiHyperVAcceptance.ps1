...
.DESCRIPTION
    ...
    verwalteten Hyper-V-SQL-2025-Run über dessen Connection-Info an den
    providerneutralen KI-Controller und prüft RAG, Agent, Login-Cleanup sowie
    einen erneuten Lauf nach VM- und Ollama-Neustart. Der übergebene Hyper-V-Run
    wird weder erzeugt noch entfernt. Bei einem geschützten Testgruppenmitglied
    wird der Einzelneustart nicht umgangen und das Ergebnis als PARTIAL markiert.
.PARAMETER RunId
    RunId eines laufenden verwalteten Hyper-V-SQL-2025-Labs.
.PARAMETER SaPassword
    Flüchtiges SA-Kennwort des Ziel-Labs.
.PARAMETER OllamaProvider
    Container-Runtime für den scopegebundenen lokalen Ollama-Endpunkt.
.PARAMETER TimeoutSeconds
    Timeout für Modell-Downloads.
.PARAMETER KeepOllamaOnFailure
    Behält nur den erzeugten Ollama-Container nach einem Fehler.
.OUTPUTS
    SqlServerLab.AiHyperVAcceptance/1.0.
.EXAMPLE
    .\Tests\Integration\Invoke-AiHyperVAcceptance.ps1 -RunId $runId -SaPassword $password
#>
[CmdletBinding()]
param(
