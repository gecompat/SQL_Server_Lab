...
<#
.SYNOPSIS
    Führt die reale erhöhte HVR-008-Legacy-Migration eines abgeschalteten Hyper-V-Runs aus.
.DESCRIPTION
...
try {
    $result = & $module {
        param($StateRoot, $TargetDataRoot, $ExpectedRunId, $ExpectedName, $TimeoutSeconds)
        ...
        if (-not $managed -or [string]$managed.VM.State -ne 'Off') {
            throw 'HVR008_ACCEPTANCE_CANDIDATE_MUST_BE_OFF'
        }
        if (-not (Get-LabSecret -Path $runDirectory -Name 'guest-administrator-password')) {
        ...
        $managedAfter = Get-HyperVManagedVM -VMName $ExpectedName -ExpectedRunId $ExpectedRunId `
            -ExpectedScopeId ([string]$run.scopeId)
        if ([string]$managedAfter.VM.State -ne 'Off') { throw 'HVR008_ACCEPTANCE_INITIAL_STATE_NOT_RESTORED' }
