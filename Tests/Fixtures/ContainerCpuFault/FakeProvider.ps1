...
function Invoke-LabCpuFaultNative {
    ...
    switch ($Arguments[0]) {
        'inspect' {
            $idExpression=if ($Binding.Provider -eq 'podman') { '{{json .ID}}' } else { '{{json .Id}}' }
            if ($Arguments.Count -ne 4 -or $Arguments[1] -cne '--format' -or -not $Arguments[2].Contains($idExpression) -or $Arguments[2] -match '\.Config(\s|}})|\.Env') { throw 'FAKE_UNSAFE_INSPECT_PROJECTION' }
