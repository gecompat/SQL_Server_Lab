...
'@
    ...
    try {
        ...
        $confirmed=if ($timedOut -or $cancelled) { Stop-PitrChild -Process $child } else { $true }
        ...
        if ($child -and -not $confirmed) { $confirmed=Stop-PitrChild -Process $child }
        $Control.TerminationConfirmed=$confirmed
        if ($confirmed) {
            # The writer has ended before streams or handles are released.
            if (-not (Wait-PitrOutputDrain -Task @($outTask,$errTask))) {
...
function Invoke-PitrSupervisor {
    param(
        ...
        [string]$EvidenceRoot,
        [ValidateRange(1,1800)][int]$TimeoutSeconds=900,
        [ValidateRange(1,600)][int]$CleanupTimeoutSeconds=300,
        [Threading.CancellationToken]$CancellationToken=[Threading.CancellationToken]::None
    )
    $arguments=@{
        AcceptanceRunner=$AcceptanceRunner; Provider=$Provider; StateRoot=$StateRoot
    ...
    try {
        $result=Invoke-PitrSupervisedChild @arguments -TimeoutSeconds $TimeoutSeconds -Control $control -CancellationToken $CancellationToken
    }
    finally {
        # Also entered after pipeline cancellation. A failed termination never authorizes cleanup.
        if ($control.TerminationConfirmed) {
            $cleanup=Invoke-PitrSupervisedChild @arguments -Stage CLEANUP -TimeoutSeconds $CleanupTimeoutSeconds
        }
    }
