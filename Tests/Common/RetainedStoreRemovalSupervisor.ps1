function New-RetainedStoreAcceptanceRoot {
    $root=Join-Path ([IO.Path]::GetTempPath()) ('sql-lab-retained-acceptance-'+[guid]::NewGuid().ToString('N'))
...
function Invoke-RetainedStoreAcceptanceChild {
    [CmdletBinding()]
    param([string]$EvidenceRoot,[guid]$OperationId,[string]$WorkerPath,
        [ValidateSet('EXERCISE','CLEANUP')][string]$Stage,[ValidateRange(1,3600)][int]$TimeoutSeconds)
    ...
    try {
        ...
        $process=[Diagnostics.Process]::Start($start);$confirmed=$false
        ...
        if($confirmed){
            foreach($task in @($outTask,$errTask)){
                if($task){try{if(-not $task.Wait(5000)){$status='FAILED';$reason='OUTPUT_FAILED'}}catch{$status='FAILED';$reason='OUTPUT_FAILED'}}
            }
...
function Invoke-RetainedStoreAcceptanceSequence {
    [CmdletBinding()]
    param([string]$EvidenceRoot,[guid]$OperationId,[string]$WorkerPath,
    ...
    try {$primary=Invoke-RetainedStoreAcceptanceChild @parameters -Stage EXERCISE -TimeoutSeconds $TimeoutSeconds}
    finally {
        if($primary.TerminationConfirmed){
            $cleanup=Invoke-RetainedStoreAcceptanceChild @parameters -Stage CLEANUP -TimeoutSeconds $CleanupTimeoutSeconds
        }
    }
