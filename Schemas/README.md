...
| `ai-retrieval-golden-dataset.schema.json` | Versionierter synthetischer Golden-Datensatz mit gebundenen Dokumenten, Fragen, Modellen, Top-k und blockierenden Schwellen |
...
| `sql-guest-evaluation-evidence.schema.json` | Runlokaler Receipt `SqlServerLab.SqlGuestEvaluationEvidence/1.0` für gebundene, frische SQL-Gast-Evaluationsfristen; nur der Reader/Watch ist implementiert, Capture und Native-Abnahme bleiben offen |
...
| `lab-storage-residency-inventory.schema.json` | Read-only Vertrag `SqlServerLab.StorageResidencyInventory/1.0` für `Lab_Data`, Backup-Sets, Datenbankpakete, Exchange-Workspaces, Runtime-Images/-Nutzung, providerseitiges Host-Backing, externe Pfade, Retention und Cleanup-Zuordnung |
| `persistent-storage-catalog.schema.json` | Katalogvertrag `SqlServerLab.PersistentStorageCatalog/1.0` für stabile IDs, Storage-Klassen, Zustände, Referenzen, exklusive Leases und atomare Artefakt-/Exchange-Workspace-Registrierung |
...
| `persistent-storage-artifact-sync-result.schema.json` | Sanitisiertes Ergebnis `SqlServerLab.PersistentStorageArtifactSyncResult/1.0` für Preview, Apply und idempotenten No-op des öffentlichen Backup-, Paket- oder Exchange-Workspace-Syncs |
...
| `lab-maintenance-plan.schema.json` | Deterministischer `SqlServerLab.MaintenancePlan/1.0`-Vertrag für State-Drift, scopegebundene Orphans, abgelaufene Temp-Tests und explizit freizugebende Legacy-Testartefakte |
...
| `container-attach-journal.schema.json` | Striktes, geheimnisfreies Journal `SqlServerLab.ContainerAttachJournal/1.0` für kopierte Container-Attach-Payloads, SQL-Postcondition und zugehörige Recovery-Aktion |
...
| `hyperv-external-runtime-reconcile-journal.schema.json` | VM- und Zielhash-gebundenes Operationsjournal `SqlServerLab.HyperVExternalRuntimeReconcileJournal/1.0` für additive katalogisierte Python-/R-/Java-Gastinstallation, SQL-Postconditions, Desired-State-Commit und Vorwärts-Resume |
